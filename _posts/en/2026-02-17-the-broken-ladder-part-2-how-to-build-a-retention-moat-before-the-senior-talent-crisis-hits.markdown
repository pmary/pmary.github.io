...
One of my juniors was implementing a rate limiter. His first instinct was to prompt "write a rate limiter for this API." We reframed it: "I'm considering a token bucket versus a sliding window approach for rate limiting. What are the failure modes of each under bursty traffic?" Same tool, same engineer. But now the AI is surfacing trade-offs he has to evaluate, not handing him code he has to trust. The same tool that shortcuts learning can deepen it, depending entirely on how it's used.
